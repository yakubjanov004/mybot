...
This module implements application creation handlers for Controller role,
allowing controllers to create both connection requests and technical service
...
logger = setup_module_logger("controller.staff_application_creation")
...
def get_controller_staff_application_router():
    ...
    async def controller_create_connection_request(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in controller_create_connection_request: {e}", exc_info=True)
    ...
    async def controller_create_technical_service(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in controller_create_technical_service: {e}", exc_info=True)
            lang = await get_user_lang(message.from_user.id)
            error_text = "Xatolik yuz berdi" if lang == 'uz' else "Произошла ошибка"
            await message.answer(error_text)
    ...
    async def handle_controller_client_search_method(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in handle_controller_client_search_method: {e}", exc_info=True)
    ...
    async def controller_cancel_application_creation(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in controller_cancel_application_creation: {e}", exc_info=True)
            await callback.answer("Xatolik yuz berdi", show_alert=True)
    ...
    async def handle_controller_client_phone_input(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in handle_controller_client_phone_input: {e}", exc_info=True)
    ...
    async def handle_controller_client_name_input(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in handle_controller_client_name_input: {e}", exc_info=True)
    ...
    async def handle_controller_client_id_input(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Error in handle_controller_client_id_input: {e}", exc_info=True)
